...
> **Source keys are canonical manifest keys.** Files inside `OBSIDIAN_VAULT_PATH` use `vault://relative/path.md` so synced vaults work across machines with different local paths. Files outside the vault use expanded absolute paths. Never mix `~`-relative and absolute keys for the same external file. See `llm-wiki/SKILL.md` → `.manifest.json`. Repair a mixed manifest with `scripts/manifest.py normalize <vault>`.
