...
**Canonical source keys.** Source keys MUST be stored in a single canonical form: **`vault://` URIs for files inside `OBSIDIAN_VAULT_PATH`, expanded absolute paths for files outside the vault**. For example, a synced note should be keyed as `vault://journals/2026-07-05.md`, while an external agent log can remain `/Users/me/.codex/sessions/.../rollout.jsonl`. Never mix `~`-relative and absolute keys for the same external file. The manifest is keyed by the raw string, so non-canonical variants make the same file look new and cause silent re-ingestion. Use `obsidian-wiki cache-check` / `cache-update` or `scripts/manifest.py delta` / `normalize` so this canonicalization is applied consistently.
